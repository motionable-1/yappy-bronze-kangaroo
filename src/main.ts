...
const readCompositionMetadata = (player: HyperframesPlayerElement) => {
  ...
  const width = Number(composition?.dataset.width ?? DEFAULT_COMPOSITION_WIDTH);
  const height = Number(
    composition?.dataset.height ?? DEFAULT_COMPOSITION_HEIGHT,
  );
  ...
  return {
    compositionId,
    width:
      Number.isFinite(width) && width > 0 ? width : DEFAULT_COMPOSITION_WIDTH,
    height:
...
const syncCompositionFrameSize = () => {
  const { width, height } = readCompositionMetadata(player);

  appShell?.setAttribute("data-motionabl-composition-width", String(width));
  appShell?.setAttribute("data-motionabl-composition-height", String(height));
...
const calculatePlayerSize = () => {
  const { width, height } = syncCompositionFrameSize();
